...
count(dvf[dvf %>% filter(date_mutation==2021 & traite == FALSE),])
count(dvf[dvf %>% filter(date_mutation==2021 & traite == TRUE),])
count(dvf[dvf %>% filter(date_mutation==2023 & traite == FALSE),])
count(dvf[dvf %>% filter(date_mutation==2023 & traite == TRUE),])
...
dvf_1 <- distinct(dvf) %>% filter(sup >= prix_m2 & prix_m2 >= inf)

ggplot(data = dvf_1 %>% filter(date_mutation==2023)) + 
