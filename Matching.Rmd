...
var_aux <- var_aux[!var_aux %in% c("id_mutation", "date_mutation", "code_commune", "nom_commune", "geometry", "traite", "nearest_metro_distance", "prix_m2", 'ind_snv', "men_pauv")]

ps <- glm(paste("traite ~", paste(var_aux, collapse = " + ")), family = binomial(), dvf_23)

step(ps,direction="both")

ps <- glm(traite ~ valeur_fonciere + type_local + surface_reelle_bati + 
    ind_snv + log_av45 + log_45_70 + log_ap90 + log_soc, family = binomial(), dvf_23)
summary(ps) 
...
# 4. appariement
?matchit
ps_match <- matchit(traite ~ valeur_fonciere + type_local + surface_reelle_bati + 
    ind_snv + log_av45 + log_45_70 + log_ap90 + log_soc, data = dvf_23)
ps_match
summary(ps_match)
...
?matchit
ps_match <- matchit(traite ~ valeur_fonciere + type_local + surface_reelle_bati + 
    ind_snv + log_av45 + log_45_70 + log_ap90 + log_soc, data = dvf_23, distance = "mahalanobis")
ps_match
summary(ps_match)
...
# 4. appariement
ps_match <- matchit(traite ~ valeur_fonciere + type_local + surface_reelle_bati + 
    log(ind_snv) + log_45_70 + log_ap90 + log_soc, data = dvf_23, distance = "mahalanobis", replace = TRUE)
